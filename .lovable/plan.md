...
- No new dependencies, no redesign of components beyond the changes listed, no unrelated functionality touched.
- Verification: typecheck, production build, lint, the 88 tests, and browser checks of every nav item, both preselection paths, all three form submissions, Services reachable and rendering, and desktop/tablet/mobile layout. I will report actual results, including anything that fails or that I could not test (signed-in views still need your account). No publish until you approve the result.
