...
I will submit one real request of each of the three types, confirm each is stored with the correct inquiry type and answers, confirm the notification shows the type and the relevant answers, confirm preselection is correct arriving from each button and when moving between Services and Agent Platform, then delete my test records.
