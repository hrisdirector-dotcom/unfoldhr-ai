...
- No new dependencies, no redesign of components beyond the changes listed, no unrelated functionality touched. No routing architecture change and no sitemap change; the routing/canonical/sitemap problem is documented as a mandatory pre-launch issue for a separate release, and no screen is presented as an independently indexed page.
- Verification: typecheck, production build, lint, the 88 tests, and browser checks of every nav destination including browser Back and Forward, preselection when switching between Services and Agent Platform, all three form submissions with notifications and confirmation emails, the founding-client price and availability wording still accurate, Services reachable and rendering, and desktop/tablet/mobile layout. I will report actual results, including anything that fails or that I could not test (signed-in views still need your account). No publish.
